...
\ref{FigResource}. Next paragraph is an attempt to explain thsi
...
(assosiated) together with other organization (multiplicity is
...
understanding is key to comprehence the concepts behind VO. They allow
to discover resource and obtain desirable data. All of them are based
on existing web standards and are designed to be simple and thefor
easy to implement on existing astronomical archives. The main idea is
simple and universal: HTTP GET reqest with parametr is sent ot the
...
The constraits are expressed as a list of ampersand-delimited GET
...
\item Image Cutout Service.
  ...
  Provides rectagular regions of large images.
...
first part is base URL. The second part are paramters specifing image
properies such as position (POS), size (SIZE), etc.
...
SSA Protocol allow ot discover and obtain 1-D spectra from VO
Service. It shares many similarities to the previosly discussed SIA
Protocol.
...
VOTable document, with at least paramters POS, SIZE, TIME, BAND, and FORMAT.
...
list with avible spectra in VOTable format. This can be used in Web
...
Astronomy has allways been, by its nature, on vanguard of image
...
VOTable is flexible storage and exchange format fundamentaly
