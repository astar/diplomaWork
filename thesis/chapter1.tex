...
Rows 1--4 define XML nd VOTable schema with adequate locations
...
\noindent The process to obtain a spectrum compose of following steps:

\begin{itemize}
\item Query the resource registry.
\item Data discovery to selected service to get available resources in
  VOTable format.
...
on the character and size. Small tables can be stored in pure
XML\,\footnote{Extensible Markup Language. W3C standard. Set of rules
  for encoding documents in machine-readable form}, while large-scale
data can be referenced with the URL\,\footnote{Uniform Resource
...
versions and references to corresponding XML Schema \footnote{Define
  ...
  NG }. \textrm{TABLE} tag encapsulating tabular data. \textrm{FIELD}
tag describe identification (ID), type and precision of
columns. \textrm{DATA} tag contains data (here) in TABLEDATA format
(other types are FITS and BINARY)
...
\footnote{International Astronomical Union FITS} and available at
...
and standardization anyone can develop his own application or enable
existing\footnote{For example Astroweka or Mirage.} application to
interact with VO Services . Libraries are also available for many
programming languages enabling advanced users to interact with VO from
scripts and programs. Such diversity is healthy and probably the only
possible way to ensure natural evolution of Virtual Observatory. 
